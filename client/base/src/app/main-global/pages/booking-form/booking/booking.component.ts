...
import { HotelClientService } from 'src/app/main-global/services/hotelClient.service';
...
export class BookingComponent implements OnInit {
  ...
  async checkCoupon(couponCode: any) {
    if (couponCode) {
          ...
          await this.CouponClientService.findByCode(couponCode).subscribe({
            next: (res) => {
              ...
              if (this.hotel_Id == res.data.hotel_id || res.data.end_date === this.now) {
                ...
                this.messageService.add({ severity: 'success', summary: 'Thành công', detail: 'Đã áp dụng mã khiếu mãi' });
              } else {
                this.messageService.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Mã không khả dụng!' });
              }

            },
            error: (err) => {
              {
                this.messageService.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Sai mã hoặc mã k tồn tại!' });
              }
            }
          });
        },
      ...
      this.messageService.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Bạn phải nhập mã!' });
    }
  }
